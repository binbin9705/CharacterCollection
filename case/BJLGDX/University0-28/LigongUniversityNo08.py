...
class LigongUniversityNo08(object):
    ...
    def start(self):
        ...
        for link in self.one_url:
            ...
            counts=self.driver.find_elements(By.XPATH,'//*[@class="pageArticle"]//li')
            print(len(counts))
            for c in counts:
                ...
                time.sleep(0.5)
                photos=self.driver.find_elements(By.XPATH,'//div[@class="subcontent"]//img')
                if len(photos)!=0:
                    photo='\n'.join(map(lambda e:e.get_attribute("src"),photos))
                else:
                    photo=''
                typess = self.driver.find_elements(By.XPATH, '//div[@class="pageArticle"]//h2')
                if len(typess)!=0:
                    types='\n'.join(map(lambda e:e.text,typess))
                    types=types.split("（按姓名拼音排序）")[0]
                else:
                    types=''

                introduces=self.driver.find_elements(By.XPATH,'//div[@class="subcontent"]')
                if len(introduces)!=0:
...
if __name__ == '__main__':
    #在职教师
    one_url = [
        'https://opt.bit.edu.cn/jsdw/jsml/index.htm',
    ]
    #学校
    school='北京理工大学-光电学院'
    demo = LigongUniversityNo08(one_url,school)
    demo.start()
