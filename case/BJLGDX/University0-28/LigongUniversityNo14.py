...
class LigongUniversityNo14(object):
    ...
    def start(self):
        ...
        # print(links)
        # for link in links:
        counts=self.driver.find_elements(By.XPATH,'//*[@class="pg_rt_con"]//li')
        print(len(counts))
        for c in counts:
            ...
            photos = self.driver.find_elements(By.XPATH, '//*[@class="pg_rt_con"]//img')
            if len(photos) != 0:
                photo = '\n'.join(map(lambda e: e.get_attribute("src"), photos))
            else:
                photo = ''
            introduces=self.driver.find_elements(By.XPATH,'//*[@class="pg_rt_con"]')
            if len(introduces)!=0:
...
if __name__ == '__main__':
    #在职教师
    one_url ='https://rw.bit.edu.cn/szdw/jsjj/index.htm'
    #学校
    school='北京理工大学-人文与社会科学学院'
    demo = LigongUniversityNo14(one_url,school)
    demo.start()
