...
class LigongUniversityNo07(object):
    ...
    def start(self):
        ...
        links=[
            'https://math.bit.edu.cn/szdw/jcrc/index.htm',
            'https://math.bit.edu.cn/szdw/jgml/index.htm',
            'https://math.bit.edu.cn/szdw/yysxyjzx2022/index.htm'
        ]
        for link in links:
            self.driver.get(link)
            counts=self.driver.find_elements(By.XPATH,'//div[@class="gp-subRight-con"]//li')
            print(len(counts))
            for c in counts:
                ...
                photos=self.driver.find_elements(By.XPATH,'//div[@class="gp-subRight-con"]//img')
                if len(photos)!=0:
                    photo='\n'.join(map(lambda e:e.get_attribute("src"),photos))
                else:
                    photo=''
                typess = self.driver.find_elements(By.XPATH, '//div[@class="gp-bread gp-f14 "]//a[4]')
                if len(typess)!=0:
                    types='\n'.join(map(lambda e:e.text,typess))
                else:
                    types=''

                introduces=self.driver.find_elements(By.XPATH,'//div[@class="gp-subRight-con"]')
                if len(introduces)!=0:
                ...
                if "职称\n" in introduce:
                    ...
                    title = introduce.split("职称：")[1].split("电子邮箱：")[0]
...
if __name__ == '__main__':
    ...
    one_url = ''
    #学校
    school='北京理工大学-数学与统计学院'
    demo = LigongUniversityNo07(one_url,school)
    demo.start()
