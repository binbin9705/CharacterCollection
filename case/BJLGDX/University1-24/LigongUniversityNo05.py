...
class LigongUniversityNo05(object):
    ...
    def start(self):
        ...
        counts=self.driver.find_elements(By.XPATH,'//*[@class="picList picList03"]//li')
        print(len(counts))
        for c in counts:
            ...
            time.sleep(0.5)
            photos = self.driver.find_elements(By.XPATH, '//*[@class="leader_con"]//img')
            if len(photos) != 0:
                photo = '\n'.join(map(lambda e: e.get_attribute("src"), photos))
            else:
                photo = ''
            introduces=self.driver.find_elements(By.XPATH,'//*[@class="leader_article"]')
            if len(introduces)!=0:
...
if __name__ == '__main__':
    one_url ='https://design.bit.edu.cn/sz2/szdw/index.htm'
    #学校
    school='北京理工大学-知艺书院'
