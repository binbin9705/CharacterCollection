...
class TjXy(object):
    ...
    def start(self):
        ...
        counts=self.driver.find_elements(By.XPATH,'//div[@class="imgzhang fl"]')
        print(len(counts))
        for c in counts:
            #姓名
            name=c.find_element(By.XPATH,'.//h1//a').text
            if "系主任:"in name:
                name=name.split("系主任:")[1]

            photo=c.find_element(By.XPATH,'.//img').get_attribute('src')
            c.click()
            #职称
            titles=self.driver.find_elements(By.XPATH,'//span[contains(text(),"职 称")]')
            if len(titles)!=0:
                title='\n'.join(map(lambda e:e.text,titles))
            else:
                title=''
            #研究领域
            fields=self.driver.find_elements(By.XPATH,'//h1[contains(text(),"研究方向")]/following-sibling::*[1]')
            if len(fields)!=0:
            ...
            if len(mailboxs)!=0:
                ...
                mailbox=''
            #简介
            introduces=self.driver.find_elements(By.XPATH,'//div[@class="teacher_experience"]')
            if len(introduces)!=0:
