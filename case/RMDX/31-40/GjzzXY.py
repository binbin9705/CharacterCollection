...
class GjzzXY(object):
    ...
    def start(self):
        ...
        #类型
        nums=self.driver.find_elements(By.XPATH,'//div[@class="article-content"]//div[@class="article-content-little-title"]')
        print(len(nums))
        for n in nums:
            types=n.text
            types=types.split("(")[0]
            print(types)
            counts=n.find_elements(By.XPATH,'./following-sibling::*[1]//li')
            print(len(counts))
            for c in counts:
                #在大页拿
                ...
                phone=''
                mailbox=''
                introduces=self.driver.find_elements(By.XPATH,'//div[@class="article-detail-text"]//p')
                if len(introduces)!=0:
